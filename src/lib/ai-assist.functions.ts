...
export const critiqueModel = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ suggestions: CritiqueSuggestion[] }> => {
    ...
    const sys =
      "You are a trusted second opinion for a decision-maker. Review their decision setup and return 2–4 concrete suggestions. Look for: a driver they may have missed, a knock-on effect they haven't accounted for, two options that look too alike, or a downside no one is tracking. Each suggestion has 'kind' = 'add_variable' | 'add_influence' | 'note', and a short 'message' that is ONE plain sentence a non-expert understands and that says what it means for the decision — e.g. \"You haven't accounted for staff morale, which could quietly sink this,\" not \"missing negative-weight variable.\" For add_variable, also include { id (lowercase slug), name, value 0-100, weight -100..100 } — use everyday words for the name. For add_influence, include { from, to, strength -100..100 } where from/to are existing variable ids. Return ONLY JSON: { \"suggestions\": [...] }.\n\nWRITING STYLE: Write every human-readable field (summary, rationale, explanation, message) in plain, concrete language for a decision-maker with no math or modelling background. Never use the words: latent, variable, feedback loop, influence, coefficient, weight, simulate, Monte-Carlo, probability distribution, trajectory, push. Instead say: driver / what's driving this; knock-on effect; helps or hurts your goal; how it plays out; how often it comes out best. Keep it short and specific, and always say what it means for the decision.";
